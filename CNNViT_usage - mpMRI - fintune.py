...
setName = 'all'
trainset = DataSet(ImgRootDir=r'G:\data\Prostate(in-house)\PCaDeepSet',
...
class vision_net(nn.Module):
    def __init__(self,  **kwargs):
        ...
        # print(summary(self.myCNN, (16, 3, 200, 200)))
        self.conv0 = nn.Conv2d(576, 3, kernel_size=1, stride=1, padding=0) # 576-7 ; 48-13
        self.relu = nn.ReLU()
        self.dropout = nn.Dropout(p=0.5)
        # self.bn = nn.BatchNorm2d(3)  # 批归一化层
...
class myCNNViT_MM(nn.Module):
    def __init__(self,  **kwargs):
        ...
        # Layer
        self.conv_0 = nn.Conv2d(576, 3, kernel_size=1, stride=1, padding=0)
        ...
        self.last_linear012 = nn.Linear(27648, 6)  #576*16*3
        ...
        self.bn = nn.BatchNorm2d(48)  # 批归一化层
    ...
    def VisionNet(self,x):
        ...
        # CNN branch of T2
        out_t2 = self.last_linear0(x0)

        return out_MM,out_t2
    
    def forward(self,input):
        x0,x1,x2 = self.encoding_cnn_MM(input)
        out_MM, out_t2 = self.VisionNet([x0,x1,x2])
...
optimizer = torch.optim.Adam(model.parameters(), lr=args.base_lr,weight_decay=1e-3)

# In[]
modelPath = r'./checkpoint/0.749_model_120_2023-07-13_13-46-32.pth'
model=torch.load(modelPath)
start_epoch = 0

# if args.RESUME:
#     path_checkpoint = "./checkpoint/0.749_model_120_2023-07-13_13-46-32.pth"  # 断点路径
...
def loss_multiTask(x,y):
    ...
    w = torch.Tensor([1,0.5,0.5,0.5])
    for i in range(x_num):
        loss_cls_ = loss_fuc(x[i],y)*w[i]
        loss_cls += loss_cls_
        loss_log.append(loss_cls_)
    
    loss_overall = loss_cls + 0.5*loss_consistency
...
for args.train_epoch in range(start_epoch,args.EPOCHS):    
    ...
    for char in pbar:   
        ...
        try:
            ...
            writer.add_scalar('Loss/train_CNN_t2', loss_log[3], n_iter)            
            writer.add_scalar('Loss/val_CNN_t2', loss_val_log[3], n_iter)
            
            writer.add_scalar('Loss/train_ViT_t2', loss_log[1], n_iter)            
        ...
        # 保存整个模型  
        MaxAccumulation = 30
        if len(ACCs)<MaxAccumulation:
            continue
        current_time = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S") # 获取当前时间
        if np.mean(np.array(ACCs)[-MaxAccumulation:],axis=0)[0]>0.65:
        ...
        if args.train_epoch%5==0 and args.train_epoch != 0:
            ...
            filename = f"{round(np.mean(ACCs[-MaxAccumulation:],axis=0)[0],4)}_checkpoint_{args.train_epoch}_{current_time}.pth" # 生成文件名 
            if not os.path.exists(os.path.join('./checkpoint',filename)):
    ...
    # model.eval()
    # if args.train_epoch%10==0 and args.train_epoch!=0:
